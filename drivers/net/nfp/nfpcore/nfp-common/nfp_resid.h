/*
 * Copyright (C) 2012-2014,2017  Netronome Systems, Inc.  All rights reserved.
 *
 * @file          nfp_resid.h
 ...
 * They serve as a software view of a group of chip families, not
 * necessarily a direct mapping to actual hardware design.
 */
 ...
 *
 */
#define NFP_CHIP_FAMILY_NFP3200 0x3200	/* ARCH_YD */
...
/** Check if @p chip_family is an ARCH_YD chip.
 * @param chip_family One of NFP_CHIP_FAMILY_*
 */
#define NFP_FAMILY_IS_ARCH_YD(chip_family) \
 ...
 * @param chip_family One of NFP_CHIP_FAMILY_*
 */
#define NFP_FAMILY_ARCH(chip_family) \
	(NFP_FAMILY_IS_ARCH_TH(chip_family) ? NFP_CHIP_ARCH_TH : \
	 NFP_FAMILY_IS_ARCH_YD(chip_family) ? NFP_CHIP_ARCH_YD : -1)
 ...
 */
#define NFP3200_MEID_IS_VALID(meid) \
	((((meid) >> 4) < 5) && (((meid) >> 4) >= 0))
 ...
 * warning)
 * if a clause is always true i.e. menum >= 0 if cluster_num is type unsigned
 * int
...
#define NFP_MEID(chip_family, island, menum)          \
	(NFP_FAMILY_IS_ARCH_TH(chip_family) ?         \
		 NFP6000_MEID(island, menum) :        \
		 NFP_FAMILY_IS_ARCH_YD(chip_family) ? \
		 NFP3200_MEID(island, menum) :        \
		 -1)
...
#define NFP_MEID_ISLAND_of(chip_family, meid)         \
	(NFP_FAMILY_IS_ARCH_TH(chip_family) ?         \
		 NFP6000_MEID_ISLAND_of(meid) :       \
		 NFP_FAMILY_IS_ARCH_YD(chip_family) ? \
		 NFP3200_MEID_CLUSTER_of(meid) :      \
		 -1)
 ...
 * 0 refers to MEs 0,1, group 1 refers to MEs 2,3 etc.
 *
 * This naming/ID is being deprecated.
 *
 * @param meid   ME ID
 */
#define NFP_MEID_MEGRP_of(chip_family, meid)          \
	(NFP_FAMILY_IS_NFP6000(chip_family) ?         \
		 NFP6000_MEID_MEGRP_of(meid) :        \
		 NFP_FAMILY_IS_NFP3200(chip_family) ? \
		 NFP3200_MEID_MEGRP_of(meid) :        \
		 -1)
