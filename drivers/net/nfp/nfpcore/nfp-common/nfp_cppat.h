					    ...
					    int isld0)
{
	...
	case 0:
		if ((cpp_tgt == NFP6000_CPPTGT_VQDR) && !addr40) {
			...
			/* a valid island. */
			i = _nfp6000_decode_basic(*addr, &v, cpp_tgt, mode, addr40, isld1,
						  isld0);
			if (i != 0)
				return i; /* Full Island ID and channel bits overlap */

			/* If dest_island is invalud, the current address won't go
			 * where expected.
			 */
		...
		_u64 = _nic_mask64((iid_lsb + 5), iid_lsb, 0); /* <39:34> or <31:26> */
		*addr &= ~_u64;
...
static nfp_inline int _nfp6000_cppat_mu_locality_lsb(int mode, int addr40)
{
	switch (mode) {
	case 0:
	case 1:
					 ...
					 int isld0)
{
	...
	case 2:
		...
		return NFP_ERRNO(ENODEV);
	case 3:
		/* Only the EMU will use 40 bit addressing. Silently set the direct
		 * locality bit for everyone else. The SDK toolchain uses
		 * dest_island <= 0 to test for atypical address encodings to support
		 * access to local-island CTM with a 32-but address (high-locality
		 * is effectively ignored and just used for routing to island #0).
		 */
