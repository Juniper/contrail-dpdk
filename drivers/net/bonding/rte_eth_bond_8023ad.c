...
static uint64_t
		lacp_timer_txrx_time_counter[RTE_TXRX][BOND_MODE_8023AD_MAX_SLAVES][RTE_HISTOGRAM_MAX_RANGE]
		= {{{0}}};
static uint64_t
		lacp_pmd_txrx_time_counter[RTE_TXRX][BOND_MODE_8023AD_MAX_SLAVES][RTE_HISTOGRAM_MAX_RANGE]
		= {{{0}}};
static uint64_t
		periodic_cb_processing_time_counts[RTE_HISTOGRAM_MAX_RANGE] = {0};
static uint64_t periodic_cb_call_counts[RTE_HISTOGRAM_MAX_RANGE] = {0};
		...
		uint64_t start, uint64_t end)
{
	...
	if (diff >= RTE_PERIODIC_CB_PROCESSING_TIME_20) {
		RTE_BOND_LOG(ERR, "Warning: Periodic CB processing time was %"PRId64"ms"
				, diff);
	}

	if (diff >= RTE_PERIODIC_CB_PROCESSING_TIME_100)
		periodic_cb_processing_time_counts[0]++;
	else if (diff >= RTE_PERIODIC_CB_PROCESSING_TIME_80)
		periodic_cb_processing_time_counts[1]++;
	else if (diff >= RTE_PERIODIC_CB_PROCESSING_TIME_60)
		periodic_cb_processing_time_counts[2]++;
	else if (diff >= RTE_PERIODIC_CB_PROCESSING_TIME_40)
		periodic_cb_processing_time_counts[3]++;
	else if (diff >= RTE_PERIODIC_CB_PROCESSING_TIME_20)
		periodic_cb_processing_time_counts[4]++;
	else
		periodic_cb_processing_time_counts[5]++;
}
...
void rte_eth_bond_8023ad_periodic_cb_call_histogram(uint64_t prev, uint64_t cur)
{
	...
	if (diff>= RTE_PERIODIC_CB_CALL_TIME_150) {
		RTE_BOND_LOG(ERR, "Warning: Periodic CB function called"
				" after %"PRId64"ms", diff);
	}

	if (diff >= RTE_PERIODIC_CB_CALL_TIME_500)
		periodic_cb_call_counts[0]++;
	else if (diff >= RTE_PERIODIC_CB_CALL_TIME_400)
		periodic_cb_call_counts[1]++;
	else if (diff >= RTE_PERIODIC_CB_CALL_TIME_300)
		periodic_cb_call_counts[2]++;
	else if (diff >= RTE_PERIODIC_CB_CALL_TIME_150)
		periodic_cb_call_counts[3]++;
	else if (diff > RTE_PERIODIC_CB_CALL_TIME_100)
		periodic_cb_call_counts[4]++;
	else
		periodic_cb_call_counts[5]++;
}
		...
		uint8_t clear)
{
	...
	if (clear) {
		memset(lacp_timer_txrx_time_counter[txrx][port_id], 0,
			RTE_HISTOGRAM_MAX_RANGE*sizeof(uint64_t));
	}
...
tx_machine(struct bond_dev_private *internals, uint16_t slave_id)
{
	...
	if (internals->mode4.dedicated_queues.enabled == 0) {
		...
		if (retval != 0) {
			...
			lacp_txrx_ring_enqueue_fail_count[0][slave_id]++;
			return;
		}
		/* Timer TX Histogram */
		rte_eth_bond_8023ad_lacp_get_timer_txrx_counts(0, slave_id);
...
bond_mode_8023ad_periodic_cb(void *arg)
{
	...
	uint8_t i, slave_id;
	uint64_t start_timer = rte_rdtsc(), end_timer, cur_timer = rte_rdtsc();
	...
	for (i = 0; i < internals->active_slave_count; i++) {
		...
		if (!is_same_ether_addr(&internals->mode4.mac_addr, &port->actor.system)) {
			ether_addr_copy(&internals->mode4.mac_addr, &port->actor.system);
			if (port->aggregator_port_id == slave_id)
				SM_FLAG_SET(port, NTT);
		}
	}
	...
	for (i = 0; i < internals->active_slave_count; i++) {
		...
		if (internals->mode4.dedicated_queues.enabled == 0) {
			...
			if (retval == 0) {
				/* Timer RX Histogram */
				rte_eth_bond_8023ad_lacp_get_timer_txrx_counts(1, slave_id);
		...
		tx_diff_timer = ((tx_end_timer - tx_start_timer) * 1000) /
				rte_get_tsc_hz();
		if (tx_diff_timer > RTE_TX_MACHINE_PROCESSING_TIME_20) {
			RTE_BOND_LOG(ERR,
...
bond_mode_8023ad_mac_address_update(struct rte_eth_dev *bond_dev)
{
	...
	for (i = 0; i < internals->active_slave_count; i++) {
		...
		if (is_same_ether_addr(&internals->mode4.mac_addr, &slave->actor.system))
			continue;


		ether_addr_copy(&internals->mode4.mac_addr, &slave->actor.system);
				  ...
				  uint16_t slave_id, struct rte_mbuf *pkt)
{
	...
	if (subtype == SLOW_SUBTYPE_MARKER) {
		...
		if (internals->mode4.dedicated_queues.enabled == 0) {
			...
			if (retval != 0) {
				lacp_txrx_ring_enqueue_fail_count[1][slave_id]++;
				/* If RX fing full free lacpdu message and drop packet */
				wrn = WRN_RX_QUEUE_FULL;
				goto free_out;
			}
			/* Bond RX Histogram */
			rte_eth_bond_8023ad_lacp_get_pmd_txrx_counts(1, slave_id);
		} else
