...
static uint64_t lacpdu_tx_count[BOND_MODE_8023AD_MAX_SLAVES] = {0, };
static uint64_t lacpdu_rx_count[BOND_MODE_8023AD_MAX_SLAVES] = {0, };
	...
	if (internals->mode4.dedicated_queues.enabled == 0) {
		...
		if (pkts_sent != 1) {
			...
			return;
                ...
                lacpdu_rx_count[slave_id] += pkts_sent;
	}
		...
		if (internals->mode4.dedicated_queues.enabled == 0) {
			...
			if (rx_count == 1)
				...
				rx_machine_update(internals, slave_id, NULL);
                        lacpdu_tx_count[slave_id] += rx_count;
		}
