...
static uint64_t eal_interrupts_call_counts[RTE_EAL_HISTOGRAM_MAX_RANGE] = {0};

void rte_eal_interrupts_call_histogram(uint64_t prev, uint64_t cur);
...
rte_eal_interrupts_call_histogram(uint64_t prev, uint64_t cur) {
    ...
    if (diff >= RTE_EAL_TIME_LAPSE_150) {
        RTE_LOG(ERR, EAL,
				"Warning: EAL interrupt function called after %"PRId64"ms\n",
				diff);
    }

    if (diff >= RTE_EAL_TIME_LAPSE_1000)
        eal_interrupts_call_counts[0]++;
    else if (diff >= RTE_EAL_TIME_LAPSE_750)
        eal_interrupts_call_counts[1]++;
    else if (diff >= RTE_EAL_TIME_LAPSE_500)
        eal_interrupts_call_counts[2]++;
    else if (diff >= RTE_EAL_TIME_LAPSE_250)
        eal_interrupts_call_counts[3]++;
    else if (diff > RTE_EAL_TIME_LAPSE_100)
